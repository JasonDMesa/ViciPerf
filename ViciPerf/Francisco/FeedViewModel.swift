...
//    will upload video by calling VideoUploader Service
    func uploadVideo() async throws {
        ...
        try await Firestore.firestore().collection("lower").document().setData(["videoUrl": videoUrl])
...
//    Will fetch the videos from Firebase so that we can display it on the feed
    ...
    func fetchVideos() async throws {
        let snapshot = try await Firestore.firestore().collection("lower").getDocuments()
